...
* **풀(pool) 구성**
  ...
  $T_i$ : VQA, OCR 등 **시각 태스크**에 속하는 *unlabeled* 이미지 집합.

* **지시 생성 비용**
  각 $T_i$에는 GPT API 호출·사람 라벨 등 **고비용 절차** $F_i(\cdot)$ 이 필요.

* **목표**
  $\mathcal{D}_S\subset D,\;|\mathcal{D}_S|\!\ll\!|D|$ 를 뽑아 *그것들만* 지시를 생성하고
...
| ------------------------------------------------ | ---------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | -------------- | --- | - | - |
| **1. Task-Importance Estimation**                | Decide *how much to sample* from each task                                   | Input: 5% reference set $D_{\text{ref}}$ (with instructions) → Output: task weights $w(T_i)$ | (i) Fine-tune LVLM on small set → Reference model <br> (ii) Compute **IRS** (Instruction Relevance Score) to assess task importance              |                |     |   |   |
| **2. Task-wise Cluster-based Selection**         | Decide *which images to sample* from each task                               | Input: Remaining 95% images (no instructions) + $w(T_i)$                                     | (i) Extract features using DINOv2 <br> (ii) k-means clustering per task <br> (iii) Select representative images via **NC** (Neighbor Centrality) |                |     |   |   |
